...
void right_rotate_test(){
    printf("right_rotate_test!\n");
    rbtree * t1 = make_testcase_rbtree();
    printf("original tree\n");
    rbtree_print(t1);
    printf("right_rotate(T, 10) -- 루트 노드인 경우\n");
    right_rotate(t1, t1->root);
    rbtree_print(t1);

    rbtree * t2 = make_testcase_rbtree();
    printf("right_rotate(T, 5) -- 회전하려는 노드가 부모의 왼쪽 자식인 경우\n");
    right_rotate(t2, t2->root->left);
    rbtree_print(t2);

    rbtree * t3 = make_testcase_rbtree();
    printf("right_rotate(T, 15) -- 회전하려는 노드가 부모의 오른쪽 자식인 경우\n");
    right_rotate(t3, t3->root->right);
    rbtree_print(t3);
    printf("right rotation 테스트 종료\n");
}

void left_rotate_test(){
    printf("left_rotate_test!\n");
    rbtree *t1 = make_testcase_rbtree();
    printf("original tree\n");
    rbtree_print(t1);
    printf("left_rotate(T, 10) -- 루트 노드인 경우\n");
    left_rotate(t1, t1->root);
    rbtree_print(t1);

    rbtree *t2 = make_testcase_rbtree();
    printf("left_rotate(T, 5) -- 회전하려는 노드가 부모의 왼쪽 자식인 경우\n");
    left_rotate(t2, t2->root->left);
    rbtree_print(t2);

    rbtree * t3 = make_testcase_rbtree();
    printf("left_rotate(T, 15) -- 회전하려는 노드가 부모의 오른쪽 자식인 경우\n");
    left_rotate(t3, t3->root->right);
    rbtree_print(t3);
